...
- **Axios interceptor** - Automatically attaches JWT Bearer token to all outgoing requests
...
│   ├── pages/
│   │   ├── auth/
│   │   │   ├── Login.jsx         # Login with Doctor/Patient tabs
...
The application contains 12 pages organized by domain:

### Public Pages

| Page | File | Route | Description |
|------|------|-------|-------------|
| Login | `pages/auth/Login.jsx` | `/login` | Doctor/Patient tab switcher with JWT authentication |
...
Configures an Axios instance with the base URL and a request interceptor that automatically attaches the JWT Bearer token from localStorage to all outgoing requests.

### api/services.js

Contains all API service functions organized by domain: authentication, doctor, patient, and prescription endpoints. Each function returns an Axios promise.

### context/AuthContext.jsx

React context provider that manages authentication state including login, logout, and persistence of token, user data, and role in localStorage.
